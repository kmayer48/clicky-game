...
class App extends Component {
  ...
  // update the current score count in the naviagator bar.
  updateCurrentScore = (newCount) => {
    // set the new count as the count
    this.setState({count: newCount});
  }

  // update the top score in the navigation bar.
  updateTopScore = (newTop) => {
    // if the new top score is higher than the current.
    if (newTop > this.state.topScore) {
      // then set the state but you have to subtract 1 to account for extra click.
